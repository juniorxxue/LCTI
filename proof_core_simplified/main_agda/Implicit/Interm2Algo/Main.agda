...
data WFC : Env n m → Counter → Set where
  wfc-z : WF1 Γ
        → WFC Γ Z
  wfc-∞ : WF2 Γ
        → WFC Γ ∞
...
complete-s {j = 𝕚 j} (s-∀l s ic fd upC upD) (wfc-𝕚 x) (⊆∀-I ext) ~j'@(~I {Σ = Σ} {e = e} ⊢e ~j)
...
complete-s {j = 𝕚 j} (s-∀l s ic fd upC upD) wfc (⊆∀-I-no ext) (~I ⊢e ~j)
  with () ← ⊆/c-find-∋= ext Z fd
complete-s {j = 𝕚 j} (s-∀l-no-appear s ic fd upC upD ) (wfc-𝕚 x) (⊆∀-I ext) (~I ⊢e ~j) = ⊥-elim (ε-¬ε-false (find-ε-gen (⊆/c-find0 ext)) fd)
complete-s {j = 𝕚 j} (s-∀l-no-appear s ic fd upC upD ) (wfc-𝕚 x) (⊆∀-I-no ext) ~j'@(~I {Σ = Σ} {e = e} ⊢e ~j)
  with ⟨ Σ' , upΣ ⟩ ← ↑tyᶜ0-total Σ
  with ⟨ e' , upe ⟩ ← ↑tyᵉ0-total e
...
complete-s {j = 𝕔 j} (s-∀l s ic fd upC upD) (wfc-𝕔 x₁) (⊆∀-C ext) ~j'@(~C {Σ = Σ} {e = e} ⊢e ~j)
  with ⟨ Σ' , upΣ ⟩ ← ↑tyᶜ0-total Σ
  with ⟨ e' , upe ⟩ ← ↑tyᵉ0-total e
  with reg-S= r regA ← s-sregular s
...
... | special {k = #0} inA Z tail (=⟹=0 up regA₁ env) s₁ = normal (case₅ inA) (s-∀l s₁ upΣ upe upC upD)
... | special {k = #S k} inA (S^ inΓ^) tail (=⟹=S newΔ up1 regB) s₁ = special (ε-∀ inA) inΓ^ (ett-∀-𝕚 tail upC upD up1) newΔ (s-∀l s₁ upΣ upe upC upD)
complete-s {j = 𝕔 j} (s-∀l s ic fd upC upD) (wfc-𝕔 x₁) (⊆∀-C-no ext) (~C ⊢e ~j)
  with () ← ⊆/c-find-∋= ext Z fd
complete-s {j = 𝕔 j} (s-∀l-no-appear s ic fd upC upD) (wfc-𝕔 x₁) (⊆∀-C ext) (~C ⊢e ~j) = ⊥-elim (ε-¬ε-false (find-ε-gen (⊆/c-find0 ext)) fd)
complete-s {j = 𝕔 j} (s-∀l-no-appear s ic fd upC upD) (wfc-𝕔 x₁) (⊆∀-C-no ext) ~j'@(~C {Σ = Σ} {e = e} ⊢e ~j)
  with ⟨ Σ' , upΣ ⟩ ← ↑tyᶜ0-total Σ
  with ⟨ e' , upe ⟩ ← ↑tyᵉ0-total e
  with reg-S^ r ← s-sregular s
  with weaken-j~ ← (~weaken^0 (~C ⊢e ~j) (↑ty-arr upC upD) (↑tyᶜ-e upe upΣ))
