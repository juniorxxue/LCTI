...
infix 3 _⊢d²_#_≤_
data _⊢d²_#_≤_ : Env n m → Counter → Type m → Type m → Set where
  s-refl :
      (regΔ : SRegular Δ)
    → (cloA : Δ ⊢r A)
    → Δ ⊢d² Z # A ≤ A
...
... | reg-S∙ r = r
s2-sregular (s-∀l grd regA s ic fd upC upD) with s2-sregular s
... | reg-S= r regA = r
s2-sregular (s-∀l-no-appear grd regA s ic fd upC upD) with s2-sregular s
... | reg-S^ r = r
...
s2-⊢r-l (s-∀l grd regA s ic fd upC upD) = ⊢r-∀ regA
s2-⊢r-l (s-∀l-no-appear grd regA s ic fd upC upD) = ⊢r-∀ regA
...
s2-⊢r-r (s-∀l grd regA s ic fd upC upD) = ⊢r-strengthen=0 (s2-⊢r-r s) (↑ty-arr upC upD)
s2-⊢r-r (s-∀l-no-appear grd regA s ic fd upC upD) = ⊢r-strengthen^0 (s2-⊢r-r s) (↑ty-arr upC upD)
...
s2-weaken= {k = k} {T = T} (s-∀l {B = B} {A% = A%} grd regA s ic fd upC upD) new (↑ty-∀ upA) (↑ty-arr {A' = C'} {B' = D'} upB upB₁)
  ...
  = s-∀l (≫-weaken= grd (▶S= new upT upB₂) upA upA%)
         ...
         ic (↑ty-find0 fd upA) upC₁ upD₁
s2-weaken= {k = k} {T = T} (s-∀l-no-appear {A% = A%} grd regA s ic fd upC upD) new (↑ty-∀ upA) (↑ty-arr {A' = C'} {B' = D'} upB upB₁)
  with ⟨ T' , upT ⟩ ← ↑ty0-total T
--  with ⟨ B' , upB₂ ⟩ ← ↑ty-total B k
  with ⟨ C₁ , upC₁ ⟩ ← ↑ty0-total C'
  with ⟨ D₁ , upD₁ ⟩ ← ↑ty0-total D'
...
s2-weaken^ {k = k} (s-∀l {B = B} {A% = A%} grd regA s ic fd upC upD) new (↑ty-∀ upA) (↑ty-arr {A' = C'} {B' = D'} upB upB₁)
  with ⟨ B' , upB₂ ⟩ ← ↑ty-total B k
  with ⟨ C₁ , upC₁ ⟩ ← ↑ty0-total C'
  with ⟨ D₁ , upD₁ ⟩ ← ↑ty0-total D'
  with ⟨ A%' , upA% ⟩ ← ↑ty-total A% (#S k)
  = s-∀l (≫-weaken^ grd (▶S= new upB₂) upA upA%)
         ...
         ic (↑ty-find0 fd upA) upC₁ upD₁
s2-weaken^ {k = k} (s-∀l-no-appear {A% = A%} grd regA s ic fd upC upD) new (↑ty-∀ upA) (↑ty-arr {A' = C'} {B' = D'} upB upB₁)
--  with ⟨ B' , upB₂ ⟩ ← ↑ty-total B k
  with ⟨ C₁ , upC₁ ⟩ ← ↑ty0-total C'
  with ⟨ D₁ , upD₁ ⟩ ← ↑ty0-total D'
  with ⟨ A%' , upA% ⟩ ← ↑ty-total A% (#S k)
