...
examplesList = 
  ...
  , Example "A3 (uncurried)" (ETrm (TList idTyp) (ETrm chooseTypUncurry EEmpty)) (Var 1 `AppUncurry` [Nil, Var 0])
      "choose(Nil, ids)"

  , Example "A3 (Fc translation, uncurried)" (ETrm (TList idTyp) (ETrm chooseTypUncurry EEmpty)) (Var 1 `AppUncurry` [Nil `Ann` TList idTyp, Var 0])
      "choose(Nil : [∀a. a → a], ids)"
  ...
  , Example "A8 (Fc translation 2, uncurried)" (ETrm chooseTypUncurry (ETrm idTyp (ETrm auto'Typ EEmpty))) (Var 0 `AppUncurry` [TAbs (Abs (Abs (Var 3 `App` Var 1 `App` Var 0)) `Ann` TArr idTyp (TArr (TVar 0) (TVar 0))), Var 2])
      "choose(Λa. λf. λx. id f x : (∀b. b → b) → a → a, auto')"
    
  , Example "A8 (Fc translation 3, uncurried)" (ETrm chooseTypUncurry (ETrm idTyp (ETrm auto'Typ EEmpty))) (Var 0 `AppUncurry` [TAbs (AbsAnn idTyp (Var 2 `App` (Var 0 `TApp` TVar 0)) `Ann` TArr idTyp (TArr (TVar 0) (TVar 0))), Var 2])
      "choose(Λa. λf : ∀b. b → b. id f @a, auto')"
  ...
  , Example "A9 (uncurried)" (ETrm (TForall $ TUncurry [TArr (TVar 0) (TVar 0), TList (TVar 0)] (TVar 0)) (ETrm chooseTyp (ETrm idTyp (ETrm (TList idTyp) EEmpty)))) (Var 0 `AppUncurry` [Var 1 `App` Var 2, Var 3])
      "f(choose id, ids) where f : ∀a. (a → a, [a]) → a"
