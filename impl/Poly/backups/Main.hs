...
sub senv tyA CEmpty | closed senv tyA = do
  (newtyA, log') <- listen $ fullInst senv tyA
...
sub senv (TVar a) (CFullType tyA) | closed senv tyA = do
  (tyB, _log1) <- listen $ findSol senv a
  ((senv', tyA'), _log2) <- listen $ sub senv tyB (CFullType tyA)
...
sub senv tyA (CFullType (TVar a)) | closed senv tyA = do
  (tyB, _log1) <- listen $ findSol senv a
  ((senv', tyA'), _log2) <- listen $ sub senv tyA (CFullType tyB)
  tell ["[S-Sol-R] " ++ logSubFull senv tyA (CFullType (TVar a)) senv' tyA']
  tell $ indentAll _log1
  tell $ indentAll _log2
  return (senv', tyA')
sub senv (TArr tyA tyB) (CFullType (TArr tyC tyD)) = do
  ((senv1, _), _log1) <- listen $ sub senv tyC (CFullType tyA)
  ((senv2, _), _log2) <- listen $ sub senv1 tyB (CFullType tyD)
...
sub senv (TArr tyA tyB) (CTerm e h) | closed senv tyA = do
  (_ , _log1) <- listen $ infer (erase senv) (CFullType tyA) e
  ((senv', tyD), _log2) <- listen $ sub senv tyB h
...
sub senv (TArr tyA tyB) (CTerm e h) | open senv tyA = do
  (tyC, _log1) <- listen $ infer (erase senv) CEmpty e
  ((senv1, tyA'), _log2) <- listen $ sub senv tyC (CFullType tyA)
  ((senv2, tyD), _log3) <- listen $ sub senv1 tyB h
...
sub senv (TForall tyA) (CFullType (TForall tyB)) = do
  ((STyp senv', tyC), _log1) <- listen $ sub senv tyA (CFullType tyB)
...
sub senv (TForall tyA) (CTerm e h) = do
  ((senv', tyB), _log1) <- listen $ sub (SEx senv) tyA (shiftContext0 (CTerm e h))
  case senv' of
    STyp senv'' -> do
...
sub senv (TForall tyA) (CTApp tyB h) = do
  ((SSol _ senv', tyC), _log1) <- listen $ sub (SSol tyB senv) tyA (shiftContext0 h)
