...
- Check if lists / paris should be considered as generic consumers
    - or only when all the components are generic consumers?
    - of course nil is not generic consumer
...
| A5  | `id auto`                          | `id auto`                                                | ✅      |
| A6  | `id auto'`                         | `id auto'`                                               | ✅      |
| A7  | `choose id auto`                   | `choose id auto`                                         | ❌      |
| A8  | `choose id auto'`                  | `choose id auto'`                                        | ❌      |
...
| C7  | `append (single inc) (single ids)` | `append (single inc) (single ids)`                       | ❌      |
| C8  | `append (single id) ids`           | `append (single id) ids`                                 | ✅      |
| C9  | `map poly (single id)`             | `map poly (single id)`                                   | ✅      |
| C10 | `map head (single ids)`            | `map head (single ids)`                                  | ❌      |
| D1  | `app poly id`                      | `app poly id`                                            | ✅      |
| D2  | `revapp id poly`                   | `revapp id poly`                                         | ✅      |
| D3  | `runST argST`                      | `runST argST`                                            | ✅      |
| D4  | `app runST argST`                  | `app runST argST`                                        | ❌      |
| D5  | `revapp argST runST`               | `revapp argST runST`                                     | ❌      |
| E1  | `k h lst`                          | `k h lst`                                                | ❌      |
| E2  | `k (\x. h x) lst`                  | `k (/\a. \x. h x : a -> a) lst`                          | Ann    |
| E3  | `r (\x. \y. y)`                    | `r (/\a. /\b. (\x. \y. y) : a -> b -> b)`                | Ann    |
