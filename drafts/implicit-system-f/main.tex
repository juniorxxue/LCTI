...
\begin{align*}
    ...
    &\text{Environments} \quad\quad &\Psi ::=&~ . \mid \Psi, x : A \mid \Psi, a \mid \Psi, \hat{a} \mid \Psi, \hat{a}= A\\
\end{align*}
...
\begin{align*}
    &\text{Environments} \quad\quad &\Gamma ::=&~ . \mid \Gamma, A \mid \Gamma, a \mid \Gamma, \hat{a} = A \\
    &\text{Subtyping Environments} \quad\quad &\Psi :=&~ \Gamma \mid \Psi, a \mid \Psi, \hat{a} \mid \Psi , \hat{a} = A\\
\end{align*}
\begin{mathpar}
\inferrule*[lab=S-Int]
{ }
...
{\hat{a} \in \Psi \\
\Psi \vdash_{close} A}
{\Psi \vdash \hat{a} <: A \dashv [(\hat{a} = A)/\hat{a}]~\Psi \rightsquigarrow A}

\inferrule*[lab=S-Ex-L-eq]
{\hat{a} = B \in \Psi \\
\Psi \vdash B <: A \dashv \Psi' \rightsquigarrow A'\\
\Psi \vdash_{close} A}
{\Psi \vdash \hat{a} <: A \dashv \Psi' \rightsquigarrow A'}

\inferrule*[lab=S-Ex-R]
{\hat{a} \in \Psi \\
\Psi \vdash_{close} A}
{\Psi \vdash A <: \hat{a} \dashv [(\hat{a} = A)/\hat{a}]~\Psi \rightsquigarrow A}

\inferrule*[lab=S-Ex-R-eq]
{\hat{a} = B \in \Psi \\
\Psi \vdash A <: B \dashv \Psi' \rightsquigarrow A''\\
\Psi \vdash_{close} A}
{\Psi \vdash A <: \hat{a} \dashv \Psi' \rightsquigarrow A''}
...
{\Psi \vdash A \rightarrow B <: \boxed{e} \leadsto \Sigma \dashv \Psi_2 \rightsquigarrow C \rightarrow D}

\inferrule*[lab=S-$\forall$]
{\Psi, a \vdash A <: B \dashv \Psi', a\rightsquigarrow C}
{\Psi \vdash \forall a.A <: \forall a.~B \dashv \Psi' \rightsquigarrow \forall a. C}

\inferrule*[lab=S-$\forall$-L]
{\Psi, \hat{a} \vdash A <: \boxed{e} \leadsto \Sigma \dashv \Psi', \hat{a} \rightsquigarrow B}
{\Psi \vdash \forall a.A <: \boxed{e} \leadsto \Sigma \dashv \Psi' \rightsquigarrow B}

\inferrule*[lab=S-$\forall$-L-Eq]
{\Psi, \hat{a} \vdash A <: \boxed{e} \leadsto \Sigma \dashv \Psi', \hat{a}=A \rightsquigarrow B}
{\Psi \vdash \forall a. A <: \boxed{e} \leadsto \Sigma \dashv \Psi' \rightsquigarrow [A/\hat{a}]B}
\end{mathpar}
