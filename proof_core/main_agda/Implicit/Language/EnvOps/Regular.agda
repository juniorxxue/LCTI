...
open import Implicit.Language.EnvOps.Weaken.Misc
...
abstract
  ...
  ∋:=-⊢r (reg-S⋈ env) (S⋈ inΓ) = ⊢r-weaken⋈0 (∋:=-⊢r env inΓ)
  ∋:=-⊢r (reg-S, reg env) (S, inΓ) = ⊢r-weaken,0 (∋:=-⊢r reg inΓ) env
...
{-
  ...
  st-⊢r' (⊢r-∀ regB) new regA (st-∀ up st) = ⊢r-∀ (st-⊢r' regB (◀S∙ new) (⊢r-weaken∙0 regA up) st)
