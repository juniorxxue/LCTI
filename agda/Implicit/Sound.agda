...
... | subs spl₁ spl-apps spl-appst mk-j infs chks sub with spl-unique spl spl₁ | s~j sub
... | ⟨ refl , ⟨ refl , ⟨ refl , refl ⟩ ⟩ ⟩ | lessc newj s₁ s₂  =
  ⊢sub' (app-elim {!!} {!!} spl-apps spl-appst {!!} (infs-w-m infs) (chks-w-m chks))
        {!!}
--  ⊢sub' (app-elim (⊢sub' (sound-i-0 ⊢e) (s-w-m s₁)) {!!} spl-apps spl-appst {!!} (infs-w-m infs) (chks-w-m chks)) {!!}
