...
complete (⊢lam₂ ⊢e) (~I {Σ = Σ} ⊢e₁ x ~j) with ↑tmᶜ0-total Σ
... | ⟨ Σ' , upΣ ⟩ = ⊢lam₂ ⊢e₁ upΣ (complete ⊢e {!!})
complete (⊢app₁ ⊢e ⊢e₁) ~j = ⊢app (complete ⊢e (~C (complete ⊢e₁ ~∞) ~j))
complete (⊢app₂ ⊢e ⊢e₁) ~j = ⊢app (complete ⊢e (~I (complete ⊢e₁ ~Z) (reg-⊆/ (Z⋈ {!!}) {!!}) ~j))
complete (⊢sub ⊢e B≤A x j≢Z) ~j = ⊢sub (complete ⊢e ~Z) {!!} x (complete-s0 B≤A ~j) where postulate
   complete-s0 : Γ ⋈ ⊢ j # A ⌞ ≤⁺ ⌝ B
               → Γ ⋈ ⊢ ⟨ j , B ⟩ ~ Σ
               → Γ ⋈ ⊢ A ≤⁺ Σ ⊣ Γ ⋈ ↪ B
...
-- complete-s {j = 𝕚 j} (s-arr₂ s s₁) (⊆I ext ext₁) (~I ⊢e x j~Σ) = {!!}
-- -- s-term-c {!!} {!!} (subsumption0 ⊢e) {!!}
-- complete-s (s-∀l s ic fd upC upD) (⊆∀-I ext) j~'@(~I ⊢e x j~) = s-∀l (complete-s s {!ext!} {!j~'!}) {!!} {!!} upC upD
