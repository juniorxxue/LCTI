...
⊆-∋=¹' (S= inΔ) (evar-sol ext cloA) = S^ (⊆-∋=¹' inΔ ext)
...
⊆-⊢cᶜ¹ : Δ ⊢cᶜ¹ Σ
       → Γ ⊆ Δ
       → Γ ⊢cᶜ¹ Σ
⊆-⊢cᶜ¹ ⊢c-empty ext = ⊢c-empty
⊆-⊢cᶜ¹ (⊢c-τ cloA) ext = ⊢c-τ (⊆-⊢c¹ cloA ext)
⊆-⊢cᶜ¹ (⊢c-term cloΣ) ext = ⊢c-term (⊆-⊢cᶜ¹ cloΣ ext)
...
... | polar-l cloA | polar-r (⊢c-τ cloA₁) = polar-r (⊢c-τ (⊢c¹-arr cloA (⊆-⊢c¹ cloA₁ (s-⊆ s))))
s-polarity {≤ = ≤⁻} (s-arr s s₁) with s-polarity s | s-polarity s₁
... | polar-r (⊢c-τ cloA₁) | polar-l cloA = polar-l (⊢c¹-arr cloA₁ (⊆-⊢c¹ cloA (s-⊆ s)))
s-polarity (s-term-c ⊢e s) with s-polarity s
... | polar-r cloΣ = polar-r (⊢c-term cloΣ)
s-polarity (s-term-o opnA ⊢e s s₁) with s-polarity s₁
... | polar-r cloΣ = polar-r (⊢c-term (⊆-⊢cᶜ¹ cloΣ (s-⊆ s)))
...
... | polar-r (⊢c-term cloΣ) = polar-r (⊢c-term (helper cloΣ upᶜ))
  where postulate helper : Γ ,^ ⊢cᶜ¹ Σ'
                         → Σ ↑tyᶜ #0 ⇘ Σ'
                         → Γ ⊢cᶜ¹ Σ
