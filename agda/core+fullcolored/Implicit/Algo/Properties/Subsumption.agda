...
test : Γ ⊢ A ≤⁺ `p P₁ ⊣ Δ ↪ B
     → P₁ ≊P P₂
     → Γ ⊢ A ≤⁺ `p P₂ ⊣ Δ ↪ B
test (s-empty regΓ cloA grd) ≊P-Z = {!!}
test (s-term-p ss s) (≊P-S newP) = s-term-p ss (test s newP)

subsumption0 : Γ ⊢ `p P ⇒ e ⇒ A
                → Γ ⊢ `τ A ⇒ e ⇒ A
subsumption0 ⊢e with t-amatch ⊢e
... | amt-𝔼 (amt2-p mt) = subsumption ⊢e (≋E p2τ) (s-type (s-refl (reg-Z (t-env ⊢e)) (⊢r-weaken⋈0 (t-⊢r ⊢e))))

subsumption0-p : Γ ⊢ `p P₁ ⇒ e ⇒ A
               → P₁ ≊P P₂
               → Γ ⊢ `p P₂ ⇒ e ⇒ A
subsumption0-p ⊢e newP = subsumption ⊢e (≋E (p2p newP)) {!⊢to≤ ⊢e!}
...
s-refined-p s'@(s-term-o opnA conv ⊢e ss s) = s-term-c {!!} {!!} {!!} {!!}
{- with subsumption0 ⊢e
...
... | s-term-o opnA conv ⊢e₁ x r = ⊥-elim (⊢r-⊢o-false (⊢r-𝕣 (t-⊢r ⊢e₁)) opnA)
⊢to≤ (⊢lam₁ ⊢e)
  with refl ← ⊢id0 ⊢e
...
subsumption {Σ' = [ e ]↝ Σ'} (⊢lam₂ ⊢e up-c ⊢e₁) (≊S newΣ) (s-term-o opnA conv ⊢e₂ x s) = ⊥-elim (t-inf-open-false ⊢e opnA)
