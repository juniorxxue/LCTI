...
⊆/-irrev-^= (ext-arr ext ext₁) inΓ newΔ (ε-arr-r inB) = ext-arr ext (⊆/-irrev-^= ext₁ {!!} newΔ inB)
...
⊆/c-irrev-^= (⊆C ext) inΓ newΔ (f-arr-𝕔 fd) = {!!}
...
⊆/-irrev-^ (ext-arr ext ext₁) (ε-arr-r inA) newΓ
  with ⟨ Ω' , ◎Ω ⟩ ← ◎-total (⊆/-=in-=out ext (◎-∋= newΓ)) = ext-arr {!!} (⊆/-irrev-^ ext₁ inA ◎Ω)
