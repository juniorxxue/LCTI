...
open import Implicit.Interm.All hiding (t-⊢r)
...
s-find (s-term-c ap ⊢e s) inΓ inΔ = f-arr-𝕔 (s-find s inΓ inΔ)
s-find {k = k} (s-term-o {A = A} ⊢e ss s) inΓ inΔ with ε-dec {k = k} {A = A}
... | inj₁ inA  = f-arr-𝕚-l inA
... | inj₂ ¬inA = f-arr-𝕚-r (s-find s (⊆/-^in-^out (ss--⊆/ ss) ¬inA inΓ) inΔ)
...
s-find (s-term-c-n s ap ⊢e) inΓ inΔ = f-arr-𝕔 (s-find s inΓ inΔ)
s-find {k = k} (s-term-o-n {B = B} s ⊢e ss) inΓ inΔ with ε-dec {k = k} {A = B}
... | inj₁ inA = f-arr-𝕚-r (s-find s inΓ (⊆/-^in-=out {!!} inA inΓ))
... | inj₂ ¬inA = f-arr-𝕚-l (^in-=out-ε (ss--⊆/ ss) (⊆/-^in-^out {!!} ¬inA inΓ) inΔ)
...
ss-¬ε+ (s-arr-n s s₁) inΓ inΔ = let inΩ = ⊆-∋^-middle inΓ inΔ (ss-⊆ s) (ss-⊆ s₁)
...
ss-¬ε- (s-∀ s) inΓ inΔ = ¬ε-∀ (ss-¬ε- s (S∙ inΓ) (S∙ inΔ))
ss-¬ε- (s-arr-n s s₁) inΓ inΔ = let inΩ = ⊆-∋^-middle inΓ inΔ (ss-⊆ s) (ss-⊆ s₁) in ¬ε-arr (ss-¬ε+ s₁ inΩ inΔ) (ss-¬ε- s inΓ inΩ)
...
s-¬ε (s-term-c ap ⊢e s) inΓ inΔ with t-⊢rᶜ ⊢e
... | ⊢rᶜ-τ regA = ¬ε-arr (⊢c-^∈-¬ε (⊢r-≫-⊢c ap (⊢r-𝕣 regA)) inΓ) (s-¬ε s inΓ inΔ)
s-¬ε (s-term-o ⊢e ss s) inΓ inΔ = let inΩ = ⊆-∋^-middle inΓ inΔ (ss-⊆ ss) (s-⊆ s)
...
s-¬ε (s-term-c-n s ap ⊢e) inΓ inΔ with t-⊢rᶜ ⊢e
... | ⊢rᶜ-τ regA = ¬ε-arr (⊢c-^∈-¬ε (⊢r-≫-⊢c ap (⊢r-𝕣 regA)) inΔ) (s-¬ε s inΓ inΔ)
s-¬ε (s-term-o-n s ⊢e ss) inΓ inΔ = let inΩ = ⊆-∋^-middle inΓ inΔ (s-⊆ s) (ss-⊆ ss) in ¬ε-arr (ss-¬ε- ss inΩ inΔ) (s-¬ε s inΓ inΩ)
