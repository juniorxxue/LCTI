...
complete-s-n! : Δ ⊢ (`𝕟 i) # A ⌞ ≤⁻ ⌝ B
              ...
              → ⟨ i , A ⟩ ~₁ P
              → Γ ⊢ B ≤⁺ `p P ⊣ Δ ↪ A
complete-s-n! (s-refl- regΔ cloA grd) (⊆/n-Z regΓ x) ~₁Z = {!!}
complete-s-n! (s-arr-n s s₁) ext ~p = {!!}
-}
...
≫-ext-sub- grd-int cloB ext regΓ = {!!}
≫-ext-sub- (grd-var= x) cloB ext regΓ = {!!}
≫-ext-sub- (grd-var∙ x) cloB ext regΓ = {!!}
≫-ext-sub- (grd-arr grd grd₁) (⊢c-arr cloB cloB₁) (ext-arr ext ext₁) regΓ = s-arr {!≫-ext-sub+ grd cloB ? ?!} (≫-ext-sub- grd₁ cloB₁ ext₁ {!!})
≫-ext-sub- (grd-∀ grd) cloB ext regΓ = {!!}

≫-ext-sub+ grd cloB ext regΓ = {!!}


complete-s-n! : Δ ⊢ (`𝕟 i) # A ⌞ ≤⁻ ⌝ B
              → Γ ⊆ Δ w/t A w/n! i
              → Γ ⊢ A ⌞ ≤⁻ ⌝ B ⊣ Δ
complete-s-n! (s-refl- regΔ cloA grd) (⊆/n-Z regΓ x) = {!!}
complete-s-n! (s-arr-n s s₁) (⊆/n-S ext) = s-arr {!!} (complete-s-n! s₁ ext)
...
-- complete-s {j = 𝕊₍ 𝕟 i ₎ j} (s-arr₂ s s₁) (⊆/c-𝕊 (⊆/e!-𝕟 ext-n) ext) (~𝕊 (~₂p ~₁) ⊢e ~j) with ⊆/e!-openclose (⊆/e!-𝕟 ext-n)
-- ... | inj₁ opnA
--   = s-term-o opnA {!!} {!!} {!!} {!!}
-- ... | inj₂ cloA = s-term-c cloA {!!} (subsumption0 ⊢e) (complete-s s₁ {!!} ~j)
-- complete-s {j = 𝕊₍ 𝕖 ₎ j} (s-∀l s upC upD upj) ext (~𝕊 ~a ⊢e ~j) = s-∀l (complete-s s {!!} {!!}) {!!} {!!} upC upD
